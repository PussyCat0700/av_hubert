...
class AVNet(pl.LightningModule):
    ...
    def __init__(self, modal, pretrainedModelPath, reqInpLen, modelargs):
        ...
        # front-end, back-end and fusion has been included in original AV-HuBERT
        wav2vecModel, _, _ = fairseq.checkpoint_utils.load_model_ensemble_and_task([pretrainedModelPath], arg_overrides={
            "apply_mask": True,
            "mask_prob": 0.5,
            ...
            "feature_grad_mult": 0.0,  # 0.0会导致不更新wav2vec，慎重选择
        })
    ...
    def subNetForward(self, inputBatch, maskw2v):
        ...
        # jointBatch: (B, T, D), float
        # inputLen: need to know what that is
        # mask: (B, T), bool
        jointBatch, mask = self.wav2vecModel.extract_finetune(
            source={'audio':audioBatch, 'video':videoBatch},
            padding_mask=audMask)  
        jointBatch = jointBatch.permute(1, 0, 2)  # (L,B,dmodel)
        jointBatch = jointBatch.transpose(0, 1).transpose(1, 2)  
        jointBatch = self.jointConv(jointBatch)  # (B,dmodel, L)
        jointBatch = jointBatch.transpose(1, 2).transpose(0, 1)  
        jointBatch = self.EncoderPositionalEncoding(jointBatch)
        jointBatch = self.jointEncoder(jointBatch, src_key_padding_mask=mask)  # (L,B,dmodel)

        return jointBatch, inputLenBatch, mask

    def forward(self, inputBatch, targetinBatch, targetLenBatch, maskw2v):
        jointBatch, inputLenBatch, mask = self.subNetForward(inputBatch, maskw2v)  # (L,B,dmodel)
        jointCTCOutputBatch = jointBatch.transpose(0, 1).transpose(1, 2)  # (L,B,dmodel)->(B,dmodel,L)
        jointCTCOutputBatch = self.jointOutputConv(jointCTCOutputBatch)  #(B,num_classes,L)  
        jointCTCOutputBatch = jointCTCOutputBatch.transpose(1, 2).transpose(0, 1)  # (L, B, num_classes)
        jointCTCOutputBatch = F.log_softmax(jointCTCOutputBatch, dim=2)  #  log_softmax on num_classes
