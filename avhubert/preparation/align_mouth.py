...
def load_args(default_config=None):
    ...
    # -- mean face utils
    parser.add_argument('--mean-face', type=str, default='20words_mean_face.npy', help='reference mean face (download from: https://github.com/mpc001/Lipreading_using_Temporal_Convolutional_Networks/blob/master/preprocessing/20words_mean_face.npy)')
    # -- mouthROIs utils
...
class Align_Process(BaseProcess):
    def __init__(self, input_dir, output_dir, fids_per_shard, args):
        super().__init__(input_dir, output_dir, fids_per_shard)
        self.args = args
        
    def run(self):
        for _, filename in enumerate(tqdm(self.fids_per_shard)):

            video_pathname = os.path.join(self.input_dir, filename+'.mp4')

            landmarks_pathname = os.path.join(self.args.landmark_direc, filename+'.pkl')
            dst_pathname = os.path.join(self.output_dir, filename+'.mp4')
            ...
            if not preprocessed_landmarks:
                ...
                frame_gen = read_video(video_pathname)
                frames = [cv2.resize(x, (self.args.crop_width, self.args.crop_height)) for x in frame_gen]
                write_video_ffmpeg(frames, dst_pathname, self.args.ffmpeg)
                continue

            # -- crop
            sequence = crop_patch(video_pathname, preprocessed_landmarks, self.args.mean_face_landmarks, self.args.stablePntsIDs, self.args.STD_SIZE, window_margin=self.args.window_margin, start_idx=self.args.start_idx, stop_idx=self.args.stop_idx, crop_height=self.args.crop_height, crop_width=self.args.crop_width)
            assert sequence is not None, "cannot crop from {}.".format(filename)

            # -- save
            os.makedirs(os.path.dirname(dst_pathname), exist_ok=True)
            write_video_ffmpeg(sequence, dst_pathname, self.args.ffmpeg)

        print('Done.')
    
class Align_Process_Manager(BaseProcessManager):
    def __init__(self, root_dir, output_dir, args, fids,  nshard, logger=BaseLogger) -> None:
        super().__init__(root_dir, output_dir, fids, nshard, logger)
        # -- mean face utils
        args.mean_face_landmarks = np.load(args.mean_face)
        args.STD_SIZE = (256, 256)
        args.stablePntsIDs = [33, 36, 39, 42, 45]
...
if __name__ == '__main__':
    ...
    process_manager = Align_Process_Manager(args.video_direc, args.save_direc, args, fids, args.nshard)
    process_manager.run(Align_Process, process_manager.extra_args)
