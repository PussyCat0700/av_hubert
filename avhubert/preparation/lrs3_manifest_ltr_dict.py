...
def wrd2ltr_sentence(s:str):
    s = s.strip()
    s = s.upper()
    arr = s.split(" ")
    arr = [" ".join(list(x)) for x in arr]
    s = " | ".join(arr)
    s += " |\n"
    return s

def wrd2ltr_label_list(lrs3:str):
    """

    Args:
        lrs3 (str): lrs3 data dir containing word-level label.list generated by lrs3_prepare.py

    Returns:
        str : str path to label_ctc.list
    """
    wrd_label_list = f"{lrs3}/label.list"
    ltr_label_list = f"{lrs3}/label_ctc.list"
    sts=[]
    assert os.path.isfile(wrd_label_list) , f"{wrd_label_list} not exist -> run lrs3_prepare.py first"
    with open(wrd_label_list, mode="r") as rf:
        sts = [wrd2ltr_sentence(x) for x in rf.readlines()]
    with open(ltr_label_list, mode="w") as wf:
        wf.writelines(sts)
    return ltr_label_list
...
def main():
    ...
    parser.add_argument('--vocab-size', type=int, default=45, help='a list of valid ids')
    args = parser.parse_args()
    file_list= f"{args.lrs3}/file.list"
    label_list = wrd2ltr_label_list(args.lrs3)
    ...
    with NamedTemporaryFile(mode="w") as f:
        label_text = [ln for ln in open(label_list).readlines()]  # gt
        for t in label_text:
            f.write(t)  # writing labels to a temporary file.
    ...
    def setup_target(target_dir, train, valid, test):
        for name, data in zip(['train', 'valid', 'test'], [train, valid, test]):
            ...
            with open(f"{target_dir}/{name}.ltr", 'w') as fo:
                for _, label, _, _ in data:
                    fo.write(f"{label}")
        shutil.copyfile(vocab_path, f"{target_dir}/dict.ltr.txt")
        return

    fids, labels = [x.strip() for x in open(file_list).readlines()], [x for x in open(label_list).readlines()]
